...
- [2026-06-04] **agent_team 创建失败 ghost 节点修复** — 修复 `agent_team` 创建阶段失败（如参数校验错误）时，React Flow 中仍出现 ghost 节点的 bug。
  
  根因：`agent:task-failed` 携带 `subAgentId`（如 `team-exec-xxx`）作为 agentId，但 `agent:team-start` 使用 `teamId`（如 `team-xxx`）。ID 不匹配导致 AgentStateMachine 的 `ensureAgent` 机制为 TASK_FAILED 事件自动创建 ghost agent。两处修复：

  1. **AgentStateMachine.ts** (`transition()` 第 149 行): `TASK_FAILED` 加入 auto-create 例外列表，与 `AGENT_CREATED`/`CLEANUP_COMPLETED_TASKS` 一同跳过 `ensureAgent`
  2. **EventAdapter.ts** (`agent:task-failed` handler): 仅在 agent 已存在时才调 `AgentStateMachine.transition(TASK_FAILED)`，空回调 AsyncTaskStore → 不产生 ghost 节点
...
- 已完成: 视频异步任务管理 Option A 实施（2026-06-03）；飞书群成员自我感知修复 — Bot 主动注册到追踪缓存（2026-06-05）；Prompt 能力发现与质量闸门轻量闭环改造（2026-06-02）；agent/tool/scene/Skills/MCP 选择链路审查与修复（2026-06-02）；TypeScript 类型检查修复（2026-06-02）；MCP PATH 隔离问题修复（2026-06-01）；多模态生成 Phase 1 代码实现（2026-06-02）
