...
export class CanvasRenderer {
   ...
   * 绘制气泡
   */
  /** @deprecated 使用 drawThinkingBubble 替代 */
  protected drawBubble(agentPos: Point, agentRadius: number, text: string, direction: 'top' | 'right' = 'top') {
    const maxWidth = 150; // 进一步减少最大宽度，避免溢出
    const padding = 10;
    const lineHeight = 16;
    const maxLines = 3; // 最多显示 3 行，防止气泡过高溢出
    ...
    if (lines.length > maxLines) {
      ...
      // 确保最后一行不会过长
      const maxLastLineWidth = maxWidth - padding * 2 - this.ctx.measureText('…').width;
      let truncatedLine = lastLine;
      while (this.ctx.measureText(truncatedLine).width > maxLastLineWidth && truncatedLine.length > 10) {
