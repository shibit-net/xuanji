...
import { JobEditDialog, DAY_NAMES } from './scheduler/JobEditDialog';
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-1 ml-3 shrink-0">
                  <Button
                    ...
                    size="icon"
                    className="h-7 w-7"
                    title={job.enabled !== false ? t('scheduler.toggle_title_disable') : t('scheduler.toggle_title_enable')}
                  >
  ...
  return (
    <div className="p-4 h-full flex flex-col">
      ...
      {logs.length === 0 ? (
        <div className="flex flex-col items-center justify-center flex-1 text-muted-foreground">
          <Clock size={40} className="mb-3 opacity-30" />
          <p className="text-sm">{t('scheduler.empty_logs')}</p>
        </div>
      ) : (
...
function SchedulerPage({ onClose }: SchedulerPageProps) {
  ...
  const tabs: { id: TabType; label: string; icon: React.ReactNode }[] = [
    { id: 'jobs', label: t('scheduler.tab.jobs'), icon: <Clock size={16} /> },
    { id: 'logs', label: t('scheduler.tab.logs'), icon: <Calendar size={16} /> },
  ];
