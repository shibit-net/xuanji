...
璇玑将隐私和安全放在首位，所有数据完全在本地处理：

- **数据本地存储**：所有对话、记忆、配置都存储在本地，不上传到任何服务器
- **用户隔离**：多用户支持，每个用户的数据完全隔离
- **API Key 安全**：LLM API Key 加密存储在本地，仅在调用时使用
- **双层安全架构**：LLM 主动审计 + 硬编码安全守护，确保危险操作不会意外执行
- **透明权限控制**：所有文件操作、命令执行都会明确告知用户，需用户确认后执行
- **无遥测**：默认不收集任何使用数据，用户完全掌控
