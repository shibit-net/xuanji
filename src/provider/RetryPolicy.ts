...
export function isContentTooLargeError(error: unknown): boolean {
  if (error instanceof Error) {
    ...
    if (status === 400 && (msg.includes('too large') || msg.includes('too long') || msg.includes('context_length_exceeded') || msg.includes('prompt too long') || msg.includes('too many tokens') || msg.includes('token limit'))) return true;
    // 消息级检测
