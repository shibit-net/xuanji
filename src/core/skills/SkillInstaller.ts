...
export class SkillInstaller {
   ...
   *  5. 构建 Skill 对象 → 注册到 SkillRegistry
   *  6. 写入 manifest.json
   *  7. 清理临时文件
   */
  async install(options: SkillInstallOptions): Promise<SkillInstallResult> {
    ...
    try {
      ...
      // ── Step 4: 读取 SKILL.md + 解析 YAML frontmatter ─
...
function validateCategory(category: unknown): 'prompt' | 'action' | 'workflow' {
  if (category === 'prompt' || category === 'action' || category === 'workflow') {
    return category;
  }
  return 'prompt';
}
