...
export class BashTool extends BaseTool {
  ...
  readonly description = [
    'Execute bash commands in shell. Working directory and environment variables persist across calls.',
    '',
    '# Use Cases',
    '✓ Run tests, build, deploy commands',
    '✓ Git operations (commit, push, branch)',
    '✓ Install dependencies (npm install, pip install)',
    '✓ System commands (ls, ps, df)',
    '',
    '# Guidelines',
    '1. Prefer dedicated tools: read_file (NOT cat), grep (NOT grep/rg), edit_file (NOT sed)',
    '2. Long-running tasks (>30s): run_in_background=true, query results via task_output',
    '3. Multiple independent commands can be called in parallel, dependent commands use &&',
    '4. Avoid interactive commands (vim, git rebase -i), stdin interaction not supported',
    '5. Provide description parameter to explain command intent (for permission confirmation)',
    '',
    '# Git Operation Standards',
    '- Before commit: git status + git diff to review changes, git log to understand commit style',
    '- When committing: Only add specific files (avoid git add -A), use HEREDOC for multi-line messages',
    '- NEVER execute: git push --force to main/master, git reset --hard (unless explicitly requested)',
    '- NEVER skip: --no-verify, --no-gpg-sign (unless explicitly requested)',
    '',
    '# Parameter Examples',
    '- Run tests: bash({ command: "npm test", description: "Run unit tests" })',
    '- Background build: bash({ command: "npm run build", run_in_background: true, description: "Build project" })',
    '- Git commit: bash({ command: "git add src/main.ts && git commit -m \'fix: bug\'", description: "Commit fix" })',
    '',
    '# Security Notes',
    '✗ Destructive operations (rm -rf, drop table) require confirmation first',
    '✗ Do NOT include passwords or API keys in commands',
    '✗ Default timeout 120s, use run_in_background for long tasks',
  ].join('\n');
