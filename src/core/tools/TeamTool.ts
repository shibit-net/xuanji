...
export class TeamTool extends BaseTool {
  ...
  readonly description = [
    ...
    '',
    '1. For EACH team member role, call match_agent or list_agents FIRST',
    '2. Review the available preset agents and their capabilities',
    '3. PREFER using matched preset agent IDs (coder, explore, test-writer, doc-writer, plan, general-purpose)',
    '4. Use matched agent ID as members[].role parameter',
    '5. Only create custom agents (with custom system_prompt) if NO preset agent matches (score < 0.5)',
    '',
    ...
    '  // Step 1: Find preset agent for code review',
    '  match_agent({ task_description: "review code quality" })',
    '  → Found: coder (score: 0.85) ✅ Use this!',
    '',
    '  // Step 2: Find preset agent for security analysis',
    '  match_agent({ task_description: "analyze security vulnerabilities" })',
    '  → Found: explore (score: 0.78) ✅ Use this!',
    '',
    '  // Step 3: Create team with PRESET agent IDs',
    '  agent_team({',
    '    members: [',
    '      { ',
    '        id: "m1", ',
    '        role: "coder",  // ✅ Using preset agent',
    '        system_prompt: "Focus on code quality: smells, maintainability, readability."',
    '      },',
    '      { ',
    '        id: "m2", ',
    '        role: "explore",  // ✅ Using preset agent',
    '        system_prompt: "Focus on security: vulnerabilities, injection risks, auth flaws."',
    '      }',
    '    ]',
    '  })',
    '',
    '❌ AVOID creating custom agents unless necessary:',
    '  // Only do this if match_agent returns score < 0.5',
    '  { ',
    '    id: "m3", ',
    '    role: "custom-specialist",  // ⚠️ Custom agent - use sparingly',
    '    system_prompt: "You are a highly specialized expert in X..."',
    '  }',
    '',
    ...
    '  strategy: "parallel"',
    '  members: [',
    '    { id: "m1", role: "explore", capabilities: ["OAuth research"] },',
    '    { id: "m2", role: "explore", capabilities: ["JWT research"] },',
    '    { id: "m3", role: "coder", capabilities: ["code review"] }',
    '  ]',
    '',
    '═══════════════════════════════════════════════════════════════',
        ...
        items: {
          ...
          properties: {
            ...
            role: {
              type: 'string',
              description: [
                'Agent ID for this team member.',
                '',
                '⚡ REQUIRED WORKFLOW:',
                '1. Call match_agent with this member\'s task description',
                '2. If score >= 0.5 (50%), use the matched agent ID here',
                '3. If score < 0.5, use "general-purpose" or omit this field',
                '',
                'Example:',
                '  match_agent({ task_description: "review code quality" })',
                '  → coder (score: 0.85)',
                ...
                '- general-purpose — Default versatile agent (fallback)',
                '',
                'If omitted, will automatically use "general-purpose".',
              ].join('\n'),
            },
            name: {
              type: 'string',
  ...
  async execute(input: Record<string, unknown>, signal?: AbortSignal): Promise<ToolResult> {
    ...
    const membersInput = input.members as Array<{
      id: string;
      role?: AgentRoleType; // role 现在是可选的
      name?: string;
      capabilities: string[];
      priority?: number;
    ...
    const members: TeamMember[] = membersInput.map(m => ({
      id: m.id,
      role: m.role || 'general-purpose', // 如果未指定 role，默认使用 general-purpose
      name: m.name,
