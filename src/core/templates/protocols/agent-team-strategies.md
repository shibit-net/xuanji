...
### 辩论流程（P1 优化：Judge 预读模式）

```
预读阶段（Judge 独有）：
  Judge: 读取关键源码文件 → 输出事实摘要（关键行号、分支条件、边界值）

第 1 轮（完整论点）：
  - PM: 基于摘要提出完整论点（引用行号，不重复读文件）
  - Engineer: 基于摘要提出反驳论点（引用行号，不重复读文件）

第 2 轮（争议回应）：
  - PM: 仅回应争议点 + 补充新证据（引用摘要行号）
  - Engineer: 仅回应争议点 + 补充新证据

第 3 轮（终审，如未收敛）：
  - Judge: 宣读事实摘要 → 逐轮裁决 → 终审判定

Token 节省：文件读取量从 角色数×轮次×文件数 → 1次（Judge预读）+ 摘要引用
审计数据：max_rounds=3 时 Token 从 2,621K → ~556K（-79%）
```
...
输入任务描述 ↓
    ...
    |    □ 单 agent 执行预估 > 15min
    |    ↓
    |    ├─ NO → 直接执行（edit_file/bash/task 等）
    |    └─ YES → 继续策略选择
    |              ↓
    ├─ 有已知决策分歧？ → 🗣️ DEBATE
    |    例："测试策略选集中还是增量"、"前端框架选A还是B"
    |
    ├─ 需要先设计再实现？ → 🏗️ HIERARCHICAL
    |    例：架构师规划 + Worker A做X + Worker B做Y
    |
    ├─ 数据需要多阶段转换？ → 🔄 PIPELINE
    |    例：数据提取→清洗→分析→可视化
    |
    ├─ 有明确步骤依赖？ → 🔗 SEQUENTIAL
    |    例：typecheck→test→build→report
    |
    └─ 无特殊依赖 → ⚡ PARALLEL（默认最快）
         例：多文件代码审查、多模块测试编写
```
...
|------|----------|-----------|----------|--------------|--------|
| 代码分析 | 3-5 分钟 ⭐ | 6-10 分钟 | 6-8 分钟 | 6-9 分钟 | 不适用 |
| 技术选型 | 不适用 | 5-8 分钟 | 不适用 | 不适用 | 8-15 分钟 ⭐ |
| 数据处理 | 不适用 | 8-12 分钟 | 6-8 分钟 ⭐ | 不适用 | 不适用 |
| 模块分析 | 3-5 分钟 ⭐ | 6-10 分钟 | 不适用 | 6-9 分钟 | 不适用 |
| 方案对比 | 不适用 | 不适用 | 不适用 | 不适用 | 8-15 分钟 ⭐ |

### 超时计算公式（P2 优化）

```
timeout = baseTimeout × complexityFactor × roundFactor

baseTimeout:
  Debate       = 1,800,000ms（30min，固定，因为有轮次）
...
roundFactor（仅 Debate）:
  max_rounds=2 → 1.0
  max_rounds=3 → 1.5
  max_rounds=4 → 2.0
  max_rounds=5 → 3.0（不推荐）
```
