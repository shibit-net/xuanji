...
export class SessionFactory {
  ...
  async create(options: SessionOptions = {}): Promise<ChatSession> {
    ...
    const enableJarvisMode = options.enableJarvisMode !== false; // 默认启用

    log.info(`Creating session for user: ${userId}, agent: ${agentId}, jarvisMode: ${enableJarvisMode}`);
    ...
    // 6. 创建 MainAgent（贾维斯架构）或 AgentLoop（标准模式）
    let mainAgent: MainAgent | null = null;
    if (enableJarvisMode) {
      mainAgent = await this.createMainAgent(config);
    }
